...
{	
	...
	public class CharacterLadder : CharacterAbility 
	{
        ...
        protected override void Start() {
            base.Start();
            if(LevelManager.Instance.game == "platform") {
            ...
            if (LevelManager.Instance.game == "endless") {
                if (LevelManager.Instance.Players[0].GetComponent<Health>().CurrentHealth > 0 && LevelManager.Instance._isPlaying) {
                    if ((Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.UpArrow)) && !moving) {
                        targetPosition = 1;
                        CurrentLadderClimbingSpeed = Vector2.up;
                        moving = true;
                    }
                    if ((Input.GetKeyDown(KeyCode.S) || Input.GetKeyDown(KeyCode.DownArrow)) && !moving) {
                        targetPosition = -1;
                        CurrentLadderClimbingSpeed = Vector2.down;
                        moving = true;
                    }
                }

                float step = LadderClimbingSpeed * Time.deltaTime;
                _controller.SetVerticalForce(targetPosition * LadderClimbingSpeed);

                if (_controller.transform.position.y < -1.95 || _controller.transform.position.y > 2.45) {
                    moving = false;
                    CurrentLadderClimbingSpeed = Vector2.zero;
                }
            }
